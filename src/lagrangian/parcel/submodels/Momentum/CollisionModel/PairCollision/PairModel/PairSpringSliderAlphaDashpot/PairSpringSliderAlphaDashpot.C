...
#include "PairSpringSliderAlphaDashpot.H"

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

template<class CloudType>
void Foam::PairSpringSliderAlphaDashpot<CloudType>::findMinMaxProperties
(
    scalar& RMin,
    scalar& rhoMax,
    scalar& UMagMax
...
template<class CloudType>
Foam::PairSpringSliderAlphaDashpot<CloudType>::PairSpringSliderAlphaDashpot
(
...
template<class CloudType>
bool Foam::PairSpringSliderAlphaDashpot<CloudType>::controlsTimestep() const
{
    return true;
}
...
template<class CloudType>
Foam::label Foam::PairSpringSliderAlphaDashpot<CloudType>::nSubCycles() const
{
    if (!(this->owner().size()))
    {
        return 1;
...
template<class CloudType>
void Foam::PairSpringSliderAlphaDashpot<CloudType>::evaluatePair
(
...
) const
{
    ...
    if (normalOverlapMag > 0)
    {
        ...
        scalar alpha = alpha_->diameter() ? 
            alpha_->sample(R*2.0) : 
            alpha_->sample(mag(U_AB));

        scalar etaN = alpha*sqrt(M*kN)*pow025(normalOverlapMag);
